...
class ConvBlockSkip(nn.Module):
    ...
    def __init__(
        self, window_size, filter_width, input_filters, nb_filters, dilation, batch_norm
    ):
...
class DeepConvLSTM(nn.Module):
    def __init__(self, config):
        ...
        for i in range(self.nb_conv_blocks):
            ...
            if self.conv_block_type == "fixup":
                self.conv_blocks.append(
                    ConvBlockFixup(
                        self.filter_width, input_filters, self.nb_filters, self.dilation
                    )
                )
        ...
        if self.reduce_layer:
            self.reduce = nn.Conv2d(
                self.nb_filters, self.reduce_layer_output, (self.filter_width, 1)
            )
                ...
                if i == 0:
                    if self.reduce_layer:
                        self.lstm_layers.append(
                            nn.LSTM(
                                self.nb_channels * self.reduce_layer_output,
                                self.nb_units_lstm,
                            )
                        )
                    else:
                        self.lstm_layers.append(
                            nn.LSTM(
                                self.nb_channels * self.nb_filters, self.nb_units_lstm
                            )
                        )
                else:
                    self.lstm_layers.append(
                        nn.LSTM(self.nb_units_lstm, self.nb_units_lstm)
                    )
    ...
    def forward(self, x):
        ...
        if self.reduce_layer:
            x = x.reshape(
                -1, self.final_seq_len, self.nb_channels * self.reduce_layer_output
            )
        else:
